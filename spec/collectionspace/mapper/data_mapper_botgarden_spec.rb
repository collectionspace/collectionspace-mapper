...
RSpec.describe CollectionSpace::Mapper::DataMapper, :integration do
  ...
  context 'botgarden profile' do
    context 'pottag record' do
      let(:mapper_path){ 'spec/fixtures/files/mappers/release_7_0/botgarden/botgarden_3-0-0_pottag.json' }

      context 'record 1' do
        let(:datahash_path){ 'spec/fixtures/files/datahashes/botgarden/pottag1.json'}
    ...
    context 'propagation record' do
      let(:mapper_path){ 'spec/fixtures/files/mappers/release_7_0/botgarden/botgarden_3-0-0_propagation.json' }
      
      context 'record 1' do
        let(:datahash_path){ 'spec/fixtures/files/datahashes/botgarden/propagation1.json' }
        let(:fixture_path){ 'botgarden/propagation1.xml' }

        # these tests are waiting for namespace uri fixes in CCU RecordMappers
        it 'does not map unexpected fields', skip: 'something weird going on with fixture_xpaths' do
          expect(diff).to eq([])
        end
    ...
    context 'taxon record' do
      let(:mapper_path){ 'spec/fixtures/files/mappers/release_6_1/botgarden/botgarden_2-0-1_taxon-local.json' }

      context 'record 1' do
        let(:datahash_path){ 'spec/fixtures/files/datahashes/botgarden/taxon1.json' }
