...
RSpec.describe CollectionSpace::Mapper::DataMapper do
  ...
  context 'core profile' do
    ...
    context 'collectionobject record' do
      ...
      context 'overflow subgroup record with uneven subgroup values' do
        let(:datahash_path){ 'spec/fixtures/files/datahashes/core/collectionobject2.json' }
        let(:fixture_path){ 'core/collectionobject2.xml' }

        it 'mapper response includes overflow subgroup warning' do
          w = response.warnings.any?{ |w| w[:category] == :subgroup_contains_data_for_nonexistent_groups }
          expect(w).to be true
        end
        it 'mapper response includes uneven subgroup values warning' do
          w = response.warnings.any?{ |w| w[:category] == :uneven_subgroup_field_values }
          expect(w).to be true
        end
      ...
      context 'overflow subgroup record with even subgroup values' do
        let(:datahash_path){ 'spec/fixtures/files/datahashes/core/collectionobject3.json' }
    ...
    context 'media record' do
      ...
      context 'sending through the bomb emoji' do
        let(:datahash_path){ 'spec/fixtures/files/datahashes/core/media2.json' }
  ...
  context 'lhmc profile' do
    ...
    context 'person record' do
      let(:mapper_path){ 'spec/fixtures/files/mappers/release_6_1/lhmc/lhmc_3-1-1_person-local.json' }
  ...
  context 'botgarden profile' do
    ...
    context 'loanout record' do
      let(:mapper_path){ 'spec/fixtures/files/mappers/release_6_1/botgarden/botgarden_2-0-1_loanout.json' }
